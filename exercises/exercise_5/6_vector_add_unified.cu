...
int main() 
{
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        // Prefetch to device
        //TODO: Uncomment the following lines if you want to use prefetching
        //cudaMemPrefetchAsync(A[iter], size, 0, streams[stream_id]);
        //cudaMemPrefetchAsync(B[iter], size, 0, streams[stream_id]);
        //cudaMemPrefetchAsync(C[iter], size, 0, streams[stream_id]);

        vectorAdd2D<<<gridDim, blockDim, 0, streams[stream_id]>>>(
            A[iter], B[iter], C[iter], N);

        // Prefetch result back to host (optional)
        //TODO: Uncomment the following lines if you want to use prefetching
        //cudaMemPrefetchAsync(C[iter], size, cudaCpuDeviceId, streams[stream_id]);
    }
